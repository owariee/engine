...
void Camera3D::processInput(Window* win)
{
    ...
    Window::MousePosition mousePosition = win->getMousePosition();
    ...
    if (win->isKeyPressed(Window::Keys::Equal))
    {
        Camera3D::fov -= 0.25 * win->getFrameTime();
    }

    if (win->isKeyPressed(Window::Keys::Minus))
    {
        Camera3D::fov += 0.25 * win->getFrameTime();
    }
